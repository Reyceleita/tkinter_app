...
class TabTickets(ttk.Frame):
    def __init__(self, parent):
        ...
        self.tabla.grid(
            ...
            column=0,
            rowspan=2,
            columnspan=len(self.columnas),
            padx=10,
            pady=10,
            sticky="we",
        )
        scrolly.grid(row=5, column=13, rowspan=2, sticky="ens")
        # Vincular scrollbar a la tabla
        ...
        for columna in self.columnas:
            self.tabla.heading(
                columna,
                text=columna,
                command=lambda c=columna, cols=self.columnas: ordenar_tabla(
                    self.tabla, c, False, cols
                ),
            )
    ...
    # Lammar función de cargar datos y aviso de progreso
