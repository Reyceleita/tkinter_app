...
class EditarTecnico(tk.Toplevel):
    def __init__(self, parent, nombre, tabla_tecnicos, cargo_lista):
        ...
        ttk.Label(self, text="Editar técnico", font=("Arial", 18)).grid(
            row=0, column=0, columnspan=2, pady=5
        )
        ttk.Label(self, text="Nombre", font=("Arial", 12)).grid(
            row=1, column=0, padx=25, pady=5, sticky="w"
        )
        ttk.Label(self, text="Cargo", font=("Arial", 12)).grid(
            row=3, column=0, padx=25, pady=5, sticky="w"
        )
        ttk.Label(self, text="Fecha de inicio", font=("Arial", 12)).grid(
            row=1, column=1, padx=25, pady=5, sticky="e"
        )
        ttk.Label(self, text="Fecha de salida", font=("Arial", 12)).grid(
            row=3, column=1, padx=25, pady=5, sticky="e"
        )

        # Campos de entrada
        self.cargo_campo = ttk.Combobox(
            self, state="readonly", textvariable=self.cargo_db, width=17
        )
        ...
        ttk.Button(
            self,
            text="Guardar",
            command=lambda: actualizar_tecnico(
                self.id_tecnico_db.get(),
                self.cargo_campo.get(),
                self.fecha_i_campo.get(),
                self.fecha_s_campo.get(),
                self.nombre_campo.get(),
                parent,
                cargo_lista,
                tabla_tecnicos,
            ),
        ).grid(row=5, column=1, padx=25, pady=5, sticky="e")
