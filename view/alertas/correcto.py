...
class Completado(tk.Toplevel):
    def __init__(self, parent, texto):
        ...
        ttk.Label(self, text="Completado", style="Completado.TLabel").grid(
            column=0, row=0, columnspan=2
        )
        ttk.Label(self, textvariable=self.texto, style="TextCompletado.TLabel").grid(
            column=0, row=1, padx=15
        )
        ttk.Button(self, text="Aceptar", command=self.destroy).grid(
